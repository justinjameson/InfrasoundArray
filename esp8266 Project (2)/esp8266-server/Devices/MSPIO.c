...
void PrintChar(EUSCI_A_Type *UART, char c)
{
	/*Send the char through the selected UART*/
	UART_Write(UART, (uint8_t*)&c, 1);
}

void PrintString(EUSCI_A_Type *UART, char *string)
{
...
void PrintInteger(EUSCI_A_Type *UART, int integer)
{
    if(integer == 0)
    {
        PrintChar(UART, '0');
